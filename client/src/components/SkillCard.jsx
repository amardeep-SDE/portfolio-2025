...
const SkillCard = ({ skill }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative flex items-center justify-between gap-1.5 sm:gap-3 w-full p-2 sm:p-3 rounded-xl 
    ...
    >
      ...
      <div className="flex items-center gap-2 sm:gap-3 min-w-0 relative z-10">
        ...
        <div
          className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl ${skill.gradient} p-[1.5px] shrink-0 shadow-2xs group-hover:scale-108 group-hover:rotate-6 transition-all duration-300`}
        >
          <div className="w-full h-full rounded-[7px] sm:rounded-[10px] bg-white dark:bg-gray-900 flex items-center justify-center p-1 sm:p-1.5">
            <img
              src={skill.icon}
              alt={skill.name}
        ...
        <div className="min-w-0">
          ...
          <span className="text-[9.5px] sm:text-[10px] text-gray-400 dark:text-gray-500 font-medium block truncate">
            {skill.level}
          </span>
        </div>
      </div>
      ...
      <div className="shrink-0 relative z-10">
        <span className="inline-block text-[9.5px] sm:text-[10.5px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full bg-indigo-50/90 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200/70 dark:border-indigo-800/60 group-hover:bg-indigo-600 group-hover:text-white group-hover:border-indigo-600 transition-colors">
          {skill.experience}
        </span>
      </div>
    </motion.div>
